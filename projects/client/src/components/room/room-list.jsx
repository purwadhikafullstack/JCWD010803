...
export const RoomList = () => {
  ...
  const getRoomByProperty = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8000/api/room/roomByProperty/${id}?page=${1}&sort=${sort}&sortBy=${sortBy}`
      );
  ...
  const fetchRoomImages = async (roomId) => {
    try {
      const response = await axios.get(
        `http://localhost:8000/api/room/RoomImg/${roomId}?page=${page}`
      );
  ...
  useEffect(() => {
    const fetchImagesForRooms = async () => {
      const images = await Promise.all(
        room.map((item) => fetchRoomImages(item.id))
      );
        ...
        {room.length !== 0 ? (
          <div>
            {room?.map((item, index) => (
              <div
                onClick={() => {
                  toDetail(item.id, item.QTY);
                }}
                className={`w-full h-44 ${
                  item.QTY !== 0
                    ? "hover:scale-95 cursor-pointer transition-all"
                    : "cursor-not-allowed"
                } flex gap-1 mb-5 border rounded-lg`}
                key={item.id}
              >
                <div
                  className={`w-4/6 h-44 bg-white ${
                    item.QTY !== 0 ? "hidden" : "block"
                  } opacity-70 z-50 absolute flex justify-center items-center text-2xl text-gray-800`}
                >
                  Room not available
                </div>
                  ...
                  <div className="w-fit">
                    <img
                      ...
                      src={`http://localhost:8000/room/${
                        roomImages[0] ? roomImages[0][0].image : "undefined"
                      }`}
                    />
                  </div>
                  <div>
                    <img
                      ...
                      src={`http://localhost:8000/room/${
                        roomImages[0] ? roomImages[0][1].image : "undefined"
                      }`}
                    />
                    <img
                      className="w-full bg-cover h-1/3"
                      src={`http://localhost:8000/room/${
                        roomImages[0] ? roomImages[0][2].image : "undefined"
                      }`}
                    />
                    <img
                      className="w-full bg-cover h-1/3"
                      src={`http://localhost:8000/room/${
                        roomImages[0] ? roomImages[0][3].image : "undefined"
                      }`}
                    />
                  </div>
                </div>
                <div className="w-full my-auto ml-5">
                  ...
                  <div className=" mt-5 text-sm text-gray-500">
                    Thank you for logging into your account! Enjoy special
                    prices.
                  </div>
                </div>
          ...
          <div className="flex justify-center items-center w-full h-auto mt-64">
            <div>
              ...
              <div className=" font-thin text-5xl">Room is unavailable</div>
            </div>
          </div>
        )}
      </div>
