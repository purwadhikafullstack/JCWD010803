...
export const Transaction = () => {
  ...
  const getRoom = async () => {
    try {
      const response = await axios.post(
        `http://localhost:8000/api/room/roomById/${roomId}`,
      ...
      <div className="pt-44 px-20 flex">
        <div className="w-2/3">
          <div className="flex gap-5 items-center h-fit">
            ...
            <div className="flex  items-center">
              <div className="text-4xl flex font-semibold text-gray-800">
          ...
          <div className="mt-16 px-14">
            ...
            <div className="mt-2">
              {" "}
              {getDateCheckIn} {getMonthCheckIn} - {getDateCheckOut}{" "}
              {getMonthCheckOut}{" "}
            </div>
            <hr className="mt-10" />
          </div>
            ...
            {methode?.map((item) => {
              return (
                <div class="radio-container gap-5 mb-3 flex">
                  ...
                  />
                  <label className=" cursor-pointer" for="radio1">
                    {" "}
                    transfer {item.methode}{" "}
                  </label>
                </div>
              );
            })}
          ...
          <div className="px-14 mt-10">
            ...
            <ul className=" text-gray-600">
              <li className="mb-2">
                - Payment must be made within 2 hours of placing the order.
              </li>
              <li>
                - Booking cancellation can be done one day before check in date
              </li>
            </ul>
            <hr className="mt-10" />
          </div>
          <div className="mt-10 text-sm px-14 w-2/3">
            ...
            <span className=" font-bold">Payment and Cancellation Policy</span>,
            and that COMFYCRIBZ{" "}
            <span className=" font-bold">may charge my payment method</span> if
            I'm liable for damages.
          </div>
          <div className="px-14 mt-10">
            <button
              onClick={() => {
              ...
              className="p-2 text-xl bg-bgPrimary text-white rounded-md transition-all hover:bg-bgPrimaryActive cursor-pointer"
            >
              Confirm and Pay
            </button>
          </div>
        ...
        <div className=" pt-10 w-1/3">
          <div className=" top-40 sticky w-full shadow-lg rounded-lg p-5">
            <div className="flex mb-5 w-full gap-5">
              <img
                src={`http://localhost:8000/room/${
                  image[0] ? image[0].image : "undefined"
                }`}
                className="w-2/4 rounded-l-lg h-full "
              />
                      ...
                      <>
                        {room.specialPrices ? (
                            ...
                            {room.specialPrices[0].isPersent === false ? (
                              <div className=" text-gray-800 flex justify-between w-full">
                                <div className=" flex underline">
                                  {formatToRupiah(
                                    parseInt(room.specialPrices[0].specialPrice)
                                  )}{" "}
                                  X {rangeDate}
                                </div>
                                <div>
                                  {formatToRupiah(
                                    parseInt(
                                      room.specialPrices[0].specialPrice *
                                        rangeDate
                                    )
                                  )}
                                </div>
                          ...
                          <div className=" text-gray-800 flex w-full justify-between">
                            <div className=" flex underline">
                              {formatToRupiah(parseInt(room.price))} X{" "}
                              {rangeDate}
                            </div>
                            <div>
